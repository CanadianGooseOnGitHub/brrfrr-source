...
class PlayState extends MusicBeatState
{
			...
			case 'outside':
				{
						defaultCamZoom = 0.9;
						curStage = 'outside';
						var bg:FlxSprite = new FlxSprite(-300, -100).loadGraphic(Paths.image('brightfyrebg/sky'));
						bg.antialiasing = true;
						bg.scrollFactor.set(0.9, 0.9);
						bg.active = false;
						add(bg);
	
						var ground:FlxSprite = new FlxSprite(-300, -25).loadGraphic(Paths.image('brightfyrebg/ground'));
						ground.updateHitbox();
						...
						var goose:FlxSprite = new FlxSprite(0, 333);
						goose.frames = Paths.getSparrowAtlas('brightfyrebg/goose');
						goose.animation.addByPrefix('bounce', "goose bop", 24);
						goose.animation.play('bounce');
						goose.scrollFactor.set(0.8, 0.8);
						add(goose);
				}
	...
	*/
	...
	override function beatHit()
	{
		...
		if (dad.curCharacter == 'brightfyre' && SONG.song.toLowerCase() == 'bruhfyre')
			{
				if (curStep == 764)
				{
		...
		if (SONG.song.toLowerCase() == 'death')
			{
				if (curStep == 444)
				{
					dad = new Character(250, 350, 'brightfyre');
					add(dad);
				}
			}
			
		if (curSong == 'Death' && dad.curCharacter == 'gf') {
